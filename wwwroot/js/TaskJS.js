...
function ThXhilqYhSiZDrl(year, month, department) 
{
    ...
    $.ajax({
        ...
        success: function (response)
        {
            ...
            if (response.month.toString().length == 2)
            {
                $('#OcoYTyiBrpZJStB').html(response.month).removeClass('OcoYTyiBrpZJStB_');
            }
            if (response.month.toString().length == 1)
            {
...
function FCpuSdjhJZYSDbe(department) {
    if ($('#zDthMDvUyTtutDb').find(':selected').val() > 0) 
    {
        //$('#loaderID').show();
        $('.right-nav').append(createLoader()); //ładowanie
        ...
        $.ajax({
            type: 'POST',
            url: '/Tasks/CopyWorkScheduleForMonthlySchedule',
            data: {
                copyYear: $('#ahTFhNgePFSjraf').find(':selected').val(),
                copyMonth: $('#KEYeauHuNLZPWCy').find(':selected').val(),
                destinyYear: $('#EGeBKVhjMnvAsVQ').find(':selected').val(),
                destinyMonth: $('#zDthMDvUyTtutDb').find(':selected').val(),
                department: department
            },
            success: function (response)
            {
                ...
                {
                    sessionStorage.setItem('LTRXohWjonyFAsg', $('#EGeBKVhjMnvAsVQ').find(':selected').val());
                    sessionStorage.setItem('XmRbNRjSsnfRbUN', $('#zDthMDvUyTtutDb').find(':selected').val());
                    ThXhilqYhSiZDrl(sessionStorage.getItem('LTRXohWjonyFAsg'), sessionStorage.getItem('XmRbNRjSsnfRbUN'), sessionStorage.getItem('JcvzYoovBpGECWh'));
...
function epJAFqfLuWUhGtx(index) 
{
    let tableTd = $('.task_table tbody tr').eq(index).find('td').not(':first-child');
    ...
    for (let i = 0; i < tableTd.length; i++) 
    {
        ...
        array.push({ worker: w, date: d });
    }

    if (array.length > 0)
    {
        let task = document.getElementById('DFOtUXAzDWlbzYQ').value;

        $('.parent').append(createLoader()); //ładowanie
        $('#PgFRHpgQidgFCiZ').remove();

        $.ajax({
            type: 'POST',
            url: '/Tasks/TasksToColumnOrRow',
            data: {
                list: array,
                taskNameId: task
            },
...
function gWuJGcgvXfzvSQF(index) {
...
function xRaHudKcDvMHfTo(index) 
{
    ...
    if (array.length > 0) 
    {
        let from = document.getElementById('eYpvywdCgUdMWFB').value;
        let to = document.getElementById('AsLyaHDkxjuuiPP').value;
...
function FkcPRLJnRAGwylX(index) 
{
    let tableTd = $('.task_table tbody tr').eq(index).find('td').not(':first-child');
...
function RMJhcqdQAvuCOYS(t) 
{
    let tr = $(t).parent().parent().parent();
    let index = $(tr).index();

    $.ajax({
        type: 'GET',
        url: '/Tasks/HoursToRowForm',
        data: {
            index: index
        },
        success: function (response)
